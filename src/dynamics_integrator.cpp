    ...
    double u2)
{
        ...
        // 目標加速度を各成分に割り当て
        alpha(0) = Cos(Thetap + thetaT)*nu1 - Sin(Thetap + thetaT)*uact1(t)*(c(s(t))*Derivative(1)(s)(t) + Derivative(1)(thetap)(t));   // α_x
        alpha(1) = nu1*Sin(Thetap + thetaT) + Cos(Thetap + thetaT)*uact1(t)*(c(s(t))*Derivative(1)(s)(t) + Derivative(1)(thetap)(t));       // α_y (通常は運動学モデル由来)
        alpha(2) = Xalpha(2) + Xalpha(0)*c(s(t)) + Derivative(1)(c)(s(t))*Power(Derivative(1)(s)(t),2);        // α_θ
        alpha(3) = nu2(t);   // α_φ (ステア角加速度)
        // 前輪／後輪回転角加速度
        alpha(4) = (Cos(phi(t))*nu1 - Sin(phi(t))*uact1(t)*uact2(t))/wheelRadius;  // α_ψf
        alpha(5) = nu1/wheelRadius;  // α_ψr
